...
class HttpService {
    ...
    static commonParams() {
        return {
            ...
            code_store: localStorage.getItem('branch')
        }
    }
            ...
            async (error) => {
                ...
                if (status === 401 && data.message === "Unauthorized") {
                    try {
                        ...
                        if (rs.status === 200) {
                            //console.log('token refreshToken: ', rs.data)
    ...
    static async doUploadRequest(url: string, data: any) {
        ...
        return HttpService.instance.post(url, data, { headers: { 'Content-Type': 'multipart/form-data', Accept: '*/*' } })
    }
    static async doPostRequest(url: string, data: any, withAccessToken = true) {
        if (!withAccessToken) {
            ...
            return HttpService.instance.post(url, data, { headers: { 'x-api-key': import.meta.env.VITE_APP_xApiKey } })
        } else {
