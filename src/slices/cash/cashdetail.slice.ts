...
interface CashdetailState extends ActionSliceState{
    ...
    item:CashDetailModel
    pageFilters:{employee:string,from:Date,to:Date}
    currentTab:number
    gridData:any[]
}

const initialState:CashdetailState={
    verifiedList: [],
    filteredVerifiedList: [],
    item: CashDetailModel.initial('IMPORT'),
    status: 'idle',
    statusAction: 'idle',
    action: "INS",
    data: { verified: { arising: [], first: 0, last: 0 }, waiting: [],delete:[]},
...
export const cashdetailSlice = createSlice({
    name: 'cashdetail',
    initialState,
    reducers: {
        selectItem:(state, action)=>{
            const {tabIndex,item}=action.payload
            switch (tabIndex) {
                case 0:
        ...
        setFilteredVerifiedList: (state, action) => {
            state.filteredVerifiedList=action.payload
        },
        setFilteredWaitingList: (state, action) => {
            state.filteredWaitingList=action.payload
        },
        setFilteredDeleteList: (state, action) => {
            state.filteredWaitingList=action.payload
        },
...
export const { 
    selectItem,resetActionState,changeAction,setFilteredVerifiedList,setFilteredWaitingList,clearAccoutings,setPayer,clearPayer,setReason,setKeyValue,setDate,updateAccountingItem,setReceiver,clearReceiver,setPageFilters,setGridData,setCurrentTab,setFilteredDeleteList
} = cashdetailSlice.actions
export default cashdetailSlice.reducer