...
import { addPawnWarehouseItem, changeAction, deletePawnWarehouseItem, editPawnWarehouseItem, fetchAllPawnWarehouse, resetActionState, restorePawnWarehouseItem, selectItem, setFilteredList } from "../../slices/pawn/pawnwarehouse.slice";
        
export default function PawnWarehouses() {
    const dispatch = useAppDispatch()
    const pawnwarehouseState = useAppSelector((state) => state.pawnwarehouse);
    const search = useAppSelector((state) => state.header.search);
    const filteredList=pawnwarehouseState.filteredList
    const [isShowDialog, setIsShowDialog] = useState(false);
    const disableInput = !["INS", "UPD"].includes(pawnwarehouseState.action);
    const [status, setStatus] = useState<status>("ACTIVE");
    const op = useRef<any>(null);
    const formik = useFormik<PawnWarehouseModel>({
        initialValues: PawnWarehouseModel.initial(),
        validate: (data) => {
            const errors:FormikErrors<PawnWarehouseModel> = {};
            if (!data.code) {
        ...
        onSubmit: (data:PawnWarehouseModel) => {
            const submitData={
                id:data.id,
                code:data.code,
                name:data.name,
            ...
            switch (pawnwarehouseState.action) {
                case "INS":
                    dispatch(addPawnWarehouseItem(submitData));
                break
                case "UPD":
                    dispatch(editPawnWarehouseItem(submitData));
                break
                case "DEL":
                    warningWithConfirm({
                        title: "Xóa",
                        ...
                        confirm: ()=>{
                            dispatch(deletePawnWarehouseItem(data))
                        }
                    })
                break
            }
    ...
    const handleActionClick=(item:PawnWarehouseModel, action:actions)=>{ 
        if(action==="DEL"){
            warningWithConfirm({
                title: "Xóa",
                ...
                confirmButtonText: "Đồng ý",
                confirm: ()=>{
                    dispatch(deletePawnWarehouseItem(item))
                }
            })
        }
        else{
    ...
    const handleRestore = () => {
        dispatch(restorePawnWarehouseItem(formik.values));
    }
    const filterList=()=>{
        let filtered=pawnwarehouseState.list
        if(status!=="ALL"){
    ...
    useEffect(() => {
        switch (pawnwarehouseState.statusAction) {
            case 'failed':
                failed(pawnwarehouseState.error);
            ...
            case 'completed':
                ...
                dispatch(resetActionState(''));
                handleCancel()
                //formik.setValues(pawnwarehouseState.item)
                dispatch(fetchAllPawnWarehouse({}))
                break;
        }
    }, [pawnwarehouseState,dispatch])
    useEffect(() => {
        if (pawnwarehouseState.status === 'failed') {
            failed(pawnwarehouseState.error);
        }
        if(pawnwarehouseState.status==="completed"){
            filterList()
        
        }
    }, [pawnwarehouseState.status])
    useEffect(() => {
        if(search!==undefined&&search!==''){
            const filtered=pawnwarehouseState.list.filter(
    ...
    return (
        <div>
                ...
                <div className="row mb-2">
                    ...
                    <InputText
                        ...
                        onChange={(e) => {
                            ...
                            if(pawnwarehouseState.action==="INS"){
            ...
            footer={
                ...
                {pawnwarehouseState.action==='VIE'?
                    ...
                    :<>
                        ...
                        <ActionButton action={"DEL"} className={""} minimumEnable={false} label={"Xóa"} onClick={()=>handleActionClick(pawnwarehouseState.item,"DEL")}/>
                    </>
                    
                :<></>}
                <FormAction action={pawnwarehouseState.action} onClick={formik.handleSubmit}/>
                </>
            } 
            draggable={false} 
            resizeable={false} 
            ...
            <Card 
                ...
                tool={
                <div className="d-flex">
                    <ActionButton action={"INS"} className={""} onClick={() => handleActionClick(PawnWarehouseModel.initial(), "INS")} minimumEnable={true} label={"Thêm"}/>
                </div>
                }
                isPadding={true} 
                className={""} 
